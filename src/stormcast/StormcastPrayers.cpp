...
namespace StormcastEternals {
    ...
    public:
        explicit Sanction(Unit *priest) :
                DamagePrayer(priest, "Sanction", 4, 7, RAND_D3) {}
    ...
    Prayer *CreateHealingStorm(Unit *priest) {
        return new HealPrayer(priest, "Healing Storm", 3, 12, RAND_D3);
    }

    class LightningStorm : public DamagePrayer {
    public:
        explicit LightningStorm(Unit *priest) :
                DamagePrayer(priest, "Lightning Storm", 3, 12, RAND_D3) {}
    ...
    void LightningStorm::secondaryEffect(Unit *target, int round) const {
        if (target) {
            target->buffModifier(Attribute::To_Hit_Melee, -1, {GamePhase::Hero, round + 1, m_priest->owningPlayer()});
            target->buffModifier(Attribute::To_Hit_Missile, -1, {GamePhase::Hero, round + 1, m_priest->owningPlayer()});
        }
    }
